...
st.markdown("""
    ...
    function drop(ev, targetId) {
        ...
        if (draggedElement.getAttribute('data-answer') == targetElement.getAttribute('data-answer')) {
...
# Zamanlayıcı ekleyelim
if 'time_left' not in st.session_state:
    st.session_state.time_left = 60

if st.session_state.time_left > 0:
    st.markdown(f"**Kalan Süre: {st.session_state.time_left} saniye**")
    st.session_state.time_left -= 1
else:
    st.markdown("**Süre doldu!**")
...
for i, word in enumerate(kelimeler):
    st.markdown(f"""
    <div id="drag_{i}" class="box" draggable="true" ondragstart="drag(event)" data-answer="{sozluk[word]}">{word}</div>
    """, unsafe_allow_html=True)

# Tanımlar
for i, definition in enumerate(tanımlar):
    st.markdown(f"""
    <div id="drop_{i}" class="box" ondrop="drop(event, 'drop_{i}')" ondragover="allowDrop(event)" data-answer="{definition}">{definition}</div>
    """, unsafe_allow_html=True)

st.markdown('</div>', unsafe_allow_html=True)

# Skor ve tekrar deneme butonu
